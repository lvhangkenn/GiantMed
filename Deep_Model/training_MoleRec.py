...
import heapq
import numpy as np
from util_MoleRec import llprint, multi_label_metric, ddi_rate_score, get_n_params
import time
...
def generate_detailed_predictions(
    ...
    print("Starting to generate MoleRec JSON prediction report...")
    ...
    with torch.no_grad():
        for patient_idx, patient_visits in enumerate(data):
            ...
            for visit_idx, adm in enumerate(patient_visits):
                ...
                target_output, _ = model(
                    patient_data=input_visits,
                    **drug_data
                )
                ...
                for med_id, prob in top_preds:
                    predicted_meds_with_probs.append(
                        [int(med_id), float(prob)]
                    )
    ...
    print(f"MoleRec prediction report saved to: {output_path}")
    print("Done!")
...
def Test(model, model_path, device, data_test, voc_size, drug_data):
    ...
    generate_detailed_predictions(
        ...
        data=data_test,
        drug_data=drug_data,
        output_path="../data/LLM-data/MIMIC-III/MoleRec_predictions.json",
        num_patients=1058
    )
