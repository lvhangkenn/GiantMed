...
class DrugRec(nn.Module):
    def __init__(self, vocab_size, emb_dim, drug_smile, smile_sub, ddi_matrix, structure_matrix, device):
        ...
        self.query_plm1 = nn.Sequential(nn.ReLU(), nn.Linear(12 * emb_dim, 2 * emb_dim))
        self.query_plm2 = nn.Sequential(nn.ReLU(), nn.Linear(2 * emb_dim, 1 * emb_dim))
        self.query_plm = nn.Sequential(nn.ReLU(), nn.Linear(12 * emb_dim, 1 * emb_dim))
        self.query_drug_emb = nn.Sequential(nn.ReLU(), nn.Linear(3 * emb_dim, emb_dim))
        ...
        self.Wr = nn.Linear(4 * emb_dim, emb_dim,
                            bias=True)  # Linear transformation for the final concatenated representation
        ...
        self.drug_embedding = nn.Embedding(131, emb_dim)
        ...
        # plm_model_name = "D:\\biobert"
        # plm_model_name = "/mnt/data/gzx/DrugRec-main-drug_text/DrugRec-main/biobert"
        # self.plm_tokenizer = AutoTokenizer.from_pretrained(plm_model_name)
        # self.plm_model= AutoModel.from_pretrained(plm_model_name)
        # self.plm_tokenizer = BertTokenizer.from_pretrained(plm_model_name)
        # self.plm_model = BertModel.from_pretrained(plm_model_name)
        self.init_weights()
    ...
    def explore_forward(self, his_query, sub_emb_before, drug_sub_matrix, source_map, explore_mask=None):
        ...
        explore_feature, attn, norm_attn = self.explore_attn(vary_health, sub_emb_before,
                                                             sub_emb_before)  # [1,1,dim]; [1,1,sub]
    ...
    def forward(self, input):
        ...
        patient_repr = torch.cat([o1, o2], dim=-1).squeeze(
            dim=0)  # (seq, dim*2)
        # print(patient_repr.shape)

        his_query = self.query(patient_repr)
        # print("patient_repr.shape:", patient_repr.shape)
        # print("his_query.shape:", his_query.shape)
        query = his_query[-1:, :]
        # self.drug_structure_emb = self.graph_network([self.smile_sub, self.stru[0], self.stru[1], self.drug_smile],self.drug_emb_id.weight)
        ...
        # self.drug_structure_emb = self.graph_network([self.smile_sub, self.stru[0], self.stru[1], self.drug_smile],self.drug_emb_id.weight)
        # self.drug_emb_id = self.drug_emb_id
        self.drug_emb = self.graph_network([self.smile_sub, self.stru[0], self.stru[1], self.drug_smile],
                                           self.drug_emb_id.weight)

        batch_size = self.drug_emb.size(0)
        num_replace = max(1, int(batch_size * 0.05))
        replace_indices = torch.randperm(batch_size)[:num_replace].to(self.device)

        # 随机生成一些embedding行索引
        rand_emb_ids = torch.randint(0, 131, (num_replace,), device=self.device)

        # 替换
        new_drug_embs = self.drug_emb.clone()
        new_drug_embs[replace_indices] = self.drug_embedding(rand_emb_ids)

        # 再过模型
        self.drug_emb = new_drug_embs

        self.drug_emb = self.drug_emb + self.drug_emb_id.weight

        # drug_emb_temp = torch.concat((ot_structure2id, ot_text2id, self.drug_emb_id.weight), dim=1)
        # self.query_drug_emb = self.query_drug_emb.to(self.device)
        # drug_emb_temp = drug_emb_temp.to(self.device)

        # self.drug_emb=self.query_drug_emb(drug_emb_temp)
        # ===============================================================================================================add
        ...
        if len(input) != 1:
            ...
            # m3: explore forward
            explore_mask = torch.bmm(sub_indices.unsqueeze(dim=0).float().unsqueeze(1), source_map).squeeze(
                1)  # 1, sum_voc
            # explore_mask = None
            explore_prob = self.explore_forward(his_query, med, self.drug_sub_martix, source_map,
