...
from model.models_DEPOT import DrugRec, SimilarityConstraintLoss
...
class DrugTrainer():
    ...
    def get_model(self):
        model = DrugRec(self.voc_size, self.embed_dim, self.drug_smile_matrix, self.smile_subs_matrix, self.ddi_adj,
                        (self.smile_sub_recency, self.smile_sub_degree), self.device)
        model = model.to(self.device)
        # 2. 加载保存好的模型权重
        model_path = 'ckpt_DEPOT.model'  # <--- 指定你的模型文件路径
        print(f"Loading pretrained model from {model_path}...")

        # 使用 map_location 确保模型能被正确加载到当前设备（CPU或GPU）
    ...
    def test(self):
        ...
        self.generate_detailed_predictions(
            ...
            output_path="../data/LLM-data/MIMIC-III/DEPOT_predictions.json",
            num_patients=1058
        )
